...
awful.util.spawn_with_shell(os.getenv("HOME") .. "/.config/awesome/tools/binit.sh")
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "main", "work", "gfx", "im", "office", 6, 7, 8, 9 }, s, layouts[1])
end
-- }}}
...
bashets.register("cpu.sh", { widget = cpuw, format = '<span color="gray">CPU:</span>$1% ', update_time = 1, separator=','})
...
for s = 1, screen.count() do
    ...
    -- bashets widgets, only on first screen
    if s == 1 then
